...
namespace ErpUnitTests
{
    ...
    public class FacadeTests : ErpBaseTest
    {
        ...
        public void GetRequest()
        {
            ...
            Assert.AreEqual("<h1>XML Postdata required</h1>", Encoding.Default.GetString(data.Content));
        }

        [TestMethod]
        public void InvalidXml()
        {
            var request = XmlRequestFromString("<don't look at me, I'm not valid xml");
            var data = erpPlugin.CreateProduct(request);
        ...
        {
            var request = XmlRequestFromString("<request xmlns=\"http://stud12.technikum-wien.at/~if12b007/SweErpXmlSchema.xsd\"><authentication></authentication><search></search></request>");
            var data = erpPlugin.CreateProduct(request);
            Assert.AreEqual(400, data.StatusCode);
        ...
        {
            var request = XmlRequestFromString("<abc><def>ghi</def></abc>");
            var data = erpPlugin.CreateProduct(request);
            Assert.AreEqual(400, data.StatusCode);
            Assert.IsTrue(Encoding.Default.GetString(data.Content).StartsWith("<h1>Postdata schema valiation failed</h1>"));
        }
        ...
        {
            var request = XmlRequestFromString(c_contactSearchRequest.Replace("123456", "wrongPw"));
            var data = erpPlugin.CreateProduct(request);
        ...
        {
            var request = XmlRequestFromString(c_contactSearchRequest.Replace("User1", "UserXX"));
            var data = erpPlugin.CreateProduct(request);
            Assert.AreEqual(401, data.StatusCode);
            Assert.AreEqual("<h1>Invalid credentials</h1>", Encoding.Default.GetString(data.Content));
        }
        ...
        {
            var request = XmlRequestFromString(c_contactSearchRequest);
            var data = erpPlugin.CreateProduct(request);
